...
inline void expect_no_throw(const std::function<void()>& fn, const char* expr, const char* file, int line) {
    try {
        fn();
    } catch (const std::exception& e) {
        std::ostringstream oss;
...
static bool suite##_##name##_registered = [](){ test_framework::register_test(#suite "." #name, suite##_##name); return true; }(); \
static void suite##_##name()

#define EXPECT_EQ(a,b) do { const auto& _a=(a); const auto& _b=(b); test_framework::expect_eq(_a,_b,#a,#b,__FILE__,__LINE__); } while(0)
#define ASSERT_EQ EXPECT_EQ
#define EXPECT_NE(a,b) do { const auto& _a=(a); const auto& _b=(b); test_framework::expect_ne(_a,_b,#a,#b,__FILE__,__LINE__); } while(0)
#define ASSERT_NE EXPECT_NE
#define EXPECT_GT(a,b) do { const auto& _a=(a); const auto& _b=(b); test_framework::expect_gt(_a,_b,#a,#b,__FILE__,__LINE__); } while(0)
#define EXPECT_GE(a,b) do { const auto& _a=(a); const auto& _b=(b); test_framework::expect_ge(_a,_b,#a,#b,__FILE__,__LINE__); } while(0)
...
#define EXPECT_NO_THROW(expr) test_framework::expect_no_throw([&](){ expr; }, #expr, __FILE__, __LINE__)
#define EXPECT_THROW(expr, exc) test_framework::expect_throw<exc>([&](){ expr; }, #expr, __FILE__, __LINE__)